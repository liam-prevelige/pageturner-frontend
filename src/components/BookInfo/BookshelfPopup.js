...
export const BookshelfPopup = () => {
  ...
  const updateSelectedBookshelf = (name) => {
    setSelectedBookshelf(name);
    ...
    <div onClick={collapseDropdown}>
      ...
      <Modal show={show} onHide={handleClose}>
              ...
              <div className="dropdown">
                ...
                <div className={dropdownClass} aria-labelledby="dropdownMenuButton">
                  {bookshelves && bookshelves.map((bookshelfData, index) =>
                    (<div className="cursor-pointer ml-2 mb-2" key={index} onClick={() => updateSelectedBookshelf(bookshelfData.name)}>
                      {bookshelfData.name}
        ...
        <Modal.Footer>
          ...
          <button className="font-bold w-36 mr-3 text-primary-button wrap-text text-sm rounded-full shadow-sm py-2 border-2 border-primary-button transform transition-colors duration-200 hover:bg-primary-button hover:border-primary-button hover:text-white" onClick={handleClose}>
            Add to Bookshelf
          </button>
        </Modal.Footer>
      </Modal>
